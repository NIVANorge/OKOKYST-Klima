...
### a. read
* this file is produced in script "Read_OKOKYST_FerryBox.Rmd"
```{r}

dir("Datasett")

df <- read.delim ("Datasett/OKOKYST_FerryBox_cleaned.txt")

#units (from similar NIVAlab file)
#DOC	Fluorescens	KlfA	NH4_N	Nitrogen part	NO3_NO2_N	O2_2	O2	O2_metning	PO4_P	POC	Salinitet	SGR_Suspendert_gløderest	Siktdyp	SiO2	STS	Temperatur	Tetthetsanomali	TOTN	TOTN_old_EF	TOTP	TOTP_P	TSM	Turbiditet
#mg/L C	µg/l	µg/l	µg/l	µg/l N	µg/l	mg/l	ml/l	%	µg/l	µg/l C		mg/l	m	mg/l	mg/l	C	kg/m3	µg/l	µg/l	µg/l	µg/l P	mg/l	FNU
...
df$StationCode <- factor(df$StationCode, levels = c("VT4", "VT12", "VT72", "VT23", "VT45", "VT22", "VT80", "VR25", "VR23", "VT76"))
...
```{r}
# look at ranges
ggplot(df_sel, aes(DOC, fill = StationId)) + 
  geom_boxplot() 

ggsave ("Figures/Boxplot_DOC_fullrange.png", width = 8, height = 6, dpi=500)

# remove two very high DOC obs: very high DOC (25 mg/L) for VT71 in 2020, approx 18 mg/L for VT16 - remove
df_sel <- subset(df_sel, DOC < 5)
...
ggplot(df_sel, aes(Salinitet, fill = StationId)) + 
  geom_boxplot() 

ggplot(df_sel, aes(Temperatur, fill = StationId)) + 
  geom_boxplot() 

ggplot(df_sel, aes(NH4, fill = StationId)) + 
  geom_boxplot() 

ggplot(df_sel, aes(TSM, fill = StationId)) + 
  geom_boxplot() 

ggsave ("Figures/Boxplot_TSM.png", width = 8, height = 6, dpi=500)

ggplot(df_sel, aes(POC, fill = StationId)) + 
  geom_boxplot() 

ggsave ("Figures/Boxplot_POC.png", width = 8, height = 6, dpi=500)

ggplot(df_sel, aes(PN, fill = StationId)) + 
  geom_boxplot() 

ggsave ("Figures/Boxplot_PN.png", width = 8, height = 6, dpi=500)

ggplot(df_sel, aes(PP, fill = StationId)) + 
  geom_boxplot() 
